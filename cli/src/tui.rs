...
impl App {
    fn new() -> Self {
        ...
        Self {
            ...
            log: vec![
                ...
                (
                    "↑↓ select model  Enter load & chat  Tab switch pane  i import  s save  q quit"
                        .into(),
                    C_DIM,
                ),
            ],
    ...
    fn refresh_names(&mut self) {
        let sel_name = self
            .list_state
            .selected()
            .and_then(|i| self.model_names.get(i))
            .cloned();
        ...
        if self.model_names.is_empty() {
            ...
            let idx = sel_name
                ...
                .unwrap_or(0);
            self.list_state
                .select(Some(idx.min(self.model_names.len() - 1)));
        }
    }
...
fn get_float(e: &ModelEntry, key: &str) -> f32 {
    match key {
        "temp" => e.temperature.unwrap_or(0.8),
        _ => 0.0,
    }
}
...
fn get_str(e: &ModelEntry, key: &str) -> String {
    match key {
        "system" => e.system_prompt.clone().unwrap_or_default(),
        _ => String::new(),
    }
}
fn apply_str(e: &mut ModelEntry, key: &str, v: String) {
    if key == "system" {
        e.system_prompt = if v.is_empty() { None } else { Some(v) };
    }
}
...
fn ui_manager(f: &mut Frame, app: &App, area: Rect) {
    ...
    match &app.mode {
        Mode::ImportPath => draw_input_dialog(
            f,
            "Import — Step 1/2",
            "Model file path:",
            &app.edit_buf,
        ...
        Mode::ImportName => {
            draw_input_dialog(f, "Import — Step 2/2", "Model name:", &app.edit_buf, area)
        }
...
fn draw_input_dialog(f: &mut Frame, title: &str, prompt: &str, buf: &str, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            format!(" {}█", buf),
            Style::default().fg(C_WHITE).bg(C_SEL_BG),
        )),
        Line::from(""),
        Line::from(Span::styled(
    ...
    match key.code {
        ...
        Char('d') | Char('D') => {
            if app.selected_entry().is_some() {
                app.mode = Mode::Confirm(ConfirmAction::Remove);
            }
        }
        ...
        Down | Char('j') if app.focus == Focus::List => {
            let i = app.list_state.selected().unwrap_or(0);
            if i + 1 < app.model_names.len() {
                app.list_state.select(Some(i + 1));
            }
        }
        Enter if app.focus == Focus::List => {
            app.load_selected();
        }
        ...
        Down | Char('j') if app.focus == Focus::Settings => {
            if app.field_sel + 1 < FIELDS.len() {
                app.field_sel += 1;
            }
        }

        // Settings pane — adjust value
        Left if app.focus == Focus::Settings => {
            app.step_field(-1, shift);
        }
        Right if app.focus == Focus::Settings => {
            app.step_field(1, shift);
        }
